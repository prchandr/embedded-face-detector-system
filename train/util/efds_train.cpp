...
The nested pairs used are of the format pair<bool, pair<float, int>>, which
is <isFace, <weight, resultSum>>.
...
vector<float> trainWeakClassifier(vector<int> resultSumsFace, vector<int> resultSumsNonFace,
	...
	vector<float> result;
	float leftPercentFace, leftPercentNonFace;
	...
	vector< pair<bool, pair<float, int>> > classifications;

	for (int i = 0; i < resultSumsFace.size(); i++) {
		totalWeightFace += weightsFace[i];
		classifications.push_back(make_pair(true, make_pair(weightsFace[i], resultSumsFace[i])));
	}

	for (int i = 0; i < resultSumsNonFace.size(); i++) {
		totalWeightNonFace += weightsNonFace[i];
		classifications.push_back(make_pair(false, make_pair(weightsNonFace[i], resultSumsNonFace[i])));
	}
	...
	for (int i = 0; i < classifications.size(); i++) {
		...
		if (classifications[i].first) {
			leftNumFace++;
			leftWeightFace += classifications[i].second.first;
		} else {
			leftNumNonFace++;
			leftWeightNonFace += classifications[i].second.first;
		}
	...
	cout << TAG << "trainWeakClassifier() optimalThreshold: " << optimalThreshold << " polarity: " << polarity << "\n"; 
	cout << TAG << "trainWeakClassifier() error: " << minError << "\n";
...
int main (int argc, char *argv[]) {
	...
	// Check program usage
	const int arg_count = 6; /* efds_train.exe weakClassifiersDir weakClassifiersIndexFile resultSumsDir strongClassifierFile iterations */
	if (argc != arg_count) {
		...
		cerr << TAG << "main() Received: " << argc << " Expected: " << arg_count << "\n";
		return 1;
	}
	...
	vector<float> weightsFace(numFaces, 0.5 / numFaces), weightsNonFace(numNonFaces, 0.5 / numNonFaces);
	...
	for (int i = 0; i < iterations; i++) {
		...
		for (int weakClassifierIdx : weakClassifierIndices) {
			cout << TAG << "main(): weakClassifierIdx: " << weakClassifierIdx << "\n";
		...
		cout << TAG << "main(): lowestError: " << lowestError << "\n";
		cout << TAG << "main(): lowestIdx: " << lowestErrorIdx << "\n";
